...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-md p-6 flex items-center justify-between transition-transform transform hover:scale-105 duration-200 relative">
            <div>
                <h3 class="text-3xl font-extrabold text-gray-800">{{ $notificationCount }}</h3>
                <p class="text-base text-gray-500 mt-1">Notificaciones</p>
            </div>
            <div class="text-blue-500 text-4xl opacity-75">
                <i class="fas fa-bell"></i>
            </div>
            <a href="{{ route('infrastock.cleaning-staff.notifications') }}" class="absolute bottom-0 left-0 right-0 bg-blue-100 text-blue-700 text-center px-4 py-2 rounded-b-xl text-sm font-medium hover:brightness-95 transition-all duration-200">Ver Notificaciones <i class="fas fa-arrow-circle-right ml-1"></i></a>
        </div>
    </div>

    <!-- Sección de Gráficos y Listas -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <!-- Gráfico de Insumo Más Solicitado -->
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <div class="flex space-x-3">
                        <a href="{{ route('infrastock.cleaning-staff.notifications') }}" class="text-gray-500 hover:text-green-600 transition-colors duration-200 p-2 rounded-md hover:bg-gray-100"><i class="fas fa-bell text-base"></i></a>
                        <a href="{{ route('infrastock.cleaning-staff.notifications') }}" class="text-gray-500 hover:text-green-600 transition-colors duration-200 p-2 rounded-md hover:bg-gray-100"><i class="fas fa-bars text-base"></i></a>
                    </div>
                </div>
                    ...
                    @forelse($notifications->take(5) as $notification)
                        <li class="py-3 flex items-center justify-between">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-sm text-gray-600 mt-1">{{ $notification->data['message'] ?? 'Sin mensaje' }}</p>
                ...
                @if($notifications->count() > 5)
                    <div class="mt-4 text-center">
                        <a href="{{ route('infrastock.cleaning-staff.notifications') }}" class="text-green-600 hover:text-green-700 font-medium text-sm">
