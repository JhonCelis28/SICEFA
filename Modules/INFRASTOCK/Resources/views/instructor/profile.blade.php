...
<div class="container-fluid">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-md p-6">
            ...
            <form action="{{ route('infrastock.instructor.profile.update') }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="text-center p-3 bg-blue-50 rounded-lg">
                            <i class="fas fa-tools text-blue-600 text-xl mb-2"></i>
                            <p class="text-sm font-medium text-gray-700">Préstamos</p>
                            <p class="text-lg font-bold text-blue-600">{{ \Modules\INFRASTOCK\Entities\WarehouseMovement::where('user_id', $user->id)->where('item_type', 'tool')->where('role', 'Préstamo')->count() }}</p>
                        </div>
                        <div class="text-center p-3 bg-green-50 rounded-lg">
                            <i class="fas fa-check-circle text-green-600 text-xl mb-2"></i>
                            <p class="text-sm font-medium text-gray-700">Devoluciones</p>
                            <p class="text-lg font-bold text-green-600">{{ \Modules\INFRASTOCK\Entities\WarehouseMovement::where('user_id', $user->id)->where('item_type', 'tool')->where('role', 'Devolución')->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
