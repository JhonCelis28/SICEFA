...
@section('title', 'Mis Solicitudes - Ganadería INFRASTOCK')
...
<div class="bg-white rounded-xl shadow-md p-6 mb-6">
    <form method="GET" action="{{ route('infrastock.ganaderia.requests.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
...
<div id="requestModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 hidden z-50" style="overflow-y: auto;">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl my-8 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6">
            <form id="requestForm" method="POST" action="{{ route('infrastock.ganaderia.requests.store') }}" class="space-y-6">
                @csrf
...
async function loadRequestFormData() {
    try {
        const response = await fetch('{{ route("infrastock.ganaderia.requests.create") }}', {
            method: 'GET',
            headers: {
...
async function editRequest(requestId) {
    try {
        const response = await fetch(`{{ url('infrastock/ganaderia/requests') }}/${requestId}/edit`, {
            method: 'GET',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json',
        ...
        const form = document.getElementById('requestForm');
        form.action = `{{ url('infrastock/ganaderia/requests') }}/${requestId}`;
        form.method = 'POST';
