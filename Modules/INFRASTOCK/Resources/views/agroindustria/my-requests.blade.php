...
@section('title', 'Mis Solicitudes - Agroindustria INFRASTOCK')
...
<div class="bg-white rounded-xl shadow-md p-6 mb-6">
    <form method="GET" action="{{ route('infrastock.agroindustria.requests.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('infrastock.agroindustria.requests.show', $request->id) }}" class="text-blue-600 hover:text-blue-900 transition-colors duration-200 p-2 rounded-full hover:bg-blue-50" title="Ver detalles">
...
<div id="requestModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 hidden z-50" style="overflow-y: auto;">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl my-8 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6">
            <form id="requestForm" method="POST" action="{{ route('infrastock.agroindustria.requests.store') }}" class="space-y-6">
                @csrf
...
function addEquipmentToList(equipment, amount = 1, itemId = null) {
    ...
    // Si hay itemId, significa que estamos editando y usamos el ID del item
    // Si no, usamos el equipment_id para crear un nuevo item
...
function resetRequestForm() {
    ...
    // Restaurar el formulario a su estado de creación
    form.action = '{{ route("infrastock.agroindustria.requests.store") }}';
    form.method = 'POST';
    
    // Eliminar el input de método PUT si existe
...
async function editRequest(requestId) {
    try {
        const response = await fetch(`{{ url('infrastock/agroindustria/requests') }}/${requestId}/edit`, {
            method: 'GET',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json',
        ...
        // Agregar los insumos de la solicitud
        ...
        const form = document.getElementById('requestForm');
        form.action = `{{ url('infrastock/agroindustria/requests') }}/${requestId}`;
        form.method = 'POST';
