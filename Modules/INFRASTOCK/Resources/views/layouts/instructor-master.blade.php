...
<head>
    ...
    <title>@yield('title', 'INFRASTOCK - Instructor')</title>
...
<body class="flex h-screen bg-gray-100 font-sans" style="font-family: 'Plus Jakarta Sans', sans-serif;">
    ...
    <div x-data="{
        ...
        <!-- Navegación del Sidebar: Lista de enlaces específicos para Instructores -->
            ...
            <ul class="space-y-1">
                ...
                <li>
                    <a href="{{ route('infrastock.instructor.my-loans') }}" class="sidebar-menu-item font-bold @if(Request::routeIs('infrastock.instructor.my-loans')) active @endif">
                        ...
                        <span x-show="sidebarOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-x-0" x-transition:enter-end="opacity-100 transform scale-x-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform scale-x-100" x-transition:leave-end="opacity-0 transform scale-x-0" class="origin-left">Mis Préstamos</span>
                    </a>
                </li>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div class="relative">
                        <button @click="dropdownOpen = !dropdownOpen" class="flex items-center space-x-2 text-gray-800 hover:text-gray-900 focus:outline-none focus:text-gray-900 p-2 rounded-md hover:bg-gray-100 transition-colors duration-200">
                            ...
                            <div>
                                ...
                                <span class="text-xs text-gray-500 block text-left">Instructor</span>
                            </div>
        ...
        <footer class="bg-white border-t border-gray-200 p-4 text-center text-gray-600 text-sm">
            &copy; {{ date('Y') }} INFRASTOCK - Instructor. Todos los derechos reservados.
        </footer>
                                ...
                                <div class="border-t pt-4">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="text-center p-3 bg-blue-50 rounded-lg">
                                            <i class="fas fa-tools text-blue-600 text-lg mb-1"></i>
                                            <p class="text-xs font-medium text-gray-700">Préstamos</p>
                                            <p class="text-sm font-bold text-blue-600">{{ \Modules\INFRASTOCK\Entities\WarehouseMovement::where('user_id', auth()->id())->where('item_type', 'tool')->where('role', 'Préstamo')->count() }}</p>
                                        </div>
                                        <div class="text-center p-3 bg-green-50 rounded-lg">
                                            <i class="fas fa-check-circle text-green-600 text-lg mb-1"></i>
                                            <p class="text-xs font-medium text-gray-700">Devoluciones</p>
                                            <p class="text-sm font-bold text-green-600">{{ \Modules\INFRASTOCK\Entities\WarehouseMovement::where('user_id', auth()->id())->where('item_type', 'tool')->where('role', 'Devolución')->count() }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        document.getElementById('profileForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route("infrastock.instructor.profile.update") }}', {
                method: 'POST',
                body: formData,
                headers: {
