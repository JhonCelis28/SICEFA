<!--
    * @file convivencia-master.blade.php
    * @brief Plantilla de diseño principal para el Centro de Convivencia del módulo INFRASTOCK.
    *
    * Esta vista Blade define la estructura global del panel del Centro de Convivencia, utilizando
    * Tailwind CSS para los estilos y Alpine.js para la interactividad. Incluye un sidebar
    * de navegación específico para el Centro de Convivencia, una barra superior con información
    * del usuario como "Centro de Convivencia", y un área de contenido principal.
    *
...
<head>
    ...
    <title>@yield('title', 'INFRASTOCK - Centro de Convivencia')</title>
...
<body class="flex h-screen bg-gray-100 font-sans" style="font-family: 'Plus Jakarta Sans', sans-serif;">
    ...
    <div x-data="{
        ...
        <!-- Navegación del Sidebar: Lista de enlaces específicos para Centro de Convivencia -->
        <nav class="flex-1 px-2 py-8 space-y-1 custom-scrollbar overflow-y-auto">
            <ul class="space-y-1">
                <!-- Dashboard Principal -->
                <li>
                    <a href="{{ route('infrastock.convivencia.dashboard') }}" class="sidebar-menu-item font-bold @if(Request::routeIs('infrastock.convivencia.dashboard')) active @endif">
                ...
                <!-- Stock Disponible -->
                <li>
                    <a href="{{ route('infrastock.convivencia.stock') }}" class="sidebar-menu-item font-bold @if(Request::routeIs('infrastock.convivencia.stock')) active @endif">
                ...
                <!-- Mis Solicitudes -->
                <li>
                    <a href="{{ route('infrastock.convivencia.requests.index') }}" class="sidebar-menu-item font-bold @if(Request::routeIs('infrastock.convivencia.requests.*')) active @endif">
                ...
                <!-- Notificaciones -->
                <li>
                    <a href="{{ route('infrastock.convivencia.notifications') }}" class="sidebar-menu-item font-bold @if(Request::routeIs('infrastock.convivencia.notifications')) active @endif">
                ...
                <li>
                    <a href="{{ route('infrastock.convivencia.surplus-report') }}" class="sidebar-menu-item font-bold @if(Request::routeIs('infrastock.convivencia.surplus-report') || Request::routeIs('infrastock.convivencia.surplus.*')) active @endif">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div x-data="{ dropdownOpen: false, notificationsOpen: false }" class="relative flex items-center space-x-4">
                                ...
                                @if(isset($notifications) && $notifications->count() > 0)
                                    ...
                                    @if($notifications->count() > 5)
                                        <div class="p-4 text-center">
                                            <a href="{{ route('infrastock.convivencia.notifications') }}" class="text-sm text-green-600 hover:text-green-700 font-medium">
                    ...
                    <div class="relative">
                        <button @click="dropdownOpen = !dropdownOpen" class="flex items-center space-x-2 text-gray-800 hover:text-gray-900 focus:outline-none focus:text-gray-900 p-2 rounded-md hover:bg-gray-100 transition-colors duration-200">
                            ...
                            <div>
                                ...
                                <span class="text-xs text-gray-500 block text-left">Centro de Convivencia</span>
                            </div>
                    ...
                    <div x-show="dropdownOpen" @click.away="dropdownOpen = false" x-cloak class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-100">
                        ...
                        <a href="{{ route('infrastock.convivencia.logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"><i class="fas fa-sign-out-alt mr-2 text-red-500"></i> Cerrar Sesión</a>
                        <form id="logout-form" action="{{ route('infrastock.convivencia.logout') }}" method="POST" class="hidden">
                            @csrf
                        </form>
                    </div>
                </div>
            ...
            <div class="mb-6">
                ...
                <nav class="text-sm font-medium text-gray-500" aria-label="breadcrumb">
                    <ol class="flex items-center space-x-2">
                        @if (!Request::routeIs('infrastock.convivencia.dashboard'))
                        <li class="flex items-center">
                            <a href="{{ route('infrastock.convivencia.dashboard') }}" class="text-green-600 hover:text-green-800">Dashboard</a>
        ...
        <footer class="bg-white border-t border-gray-200 p-4 text-center text-gray-600 text-sm">
            &copy; {{ date('Y') }} INFRASTOCK - Centro de Convivencia. Todos los derechos reservados.
        </footer>
    ...
    <script>
        ...
        document.getElementById('profileForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route("infrastock.convivencia.profile.update") }}', {
                method: 'POST',
                body: formData,
                headers: {
