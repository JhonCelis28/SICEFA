...
<div class="container-fluid">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-md p-6">
            ...
            <form action="{{ route('infrastock.convivencia.surplus.store') }}" method="POST" class="space-y-6" id="surplusForm">
                @csrf
...
function showSurplusDetails(surplusId) {
    fetch(`{{ route('infrastock.convivencia.surplus.show', '') }}/${surplusId}`)
...
function deleteSurplus(surplusId) {
    if (confirm('¿Estás seguro de que quieres eliminar este registro de sobrante?')) {
        ...
        form.method = 'POST';
        form.action = `{{ route('infrastock.convivencia.surplus.destroy', '') }}/${surplusId}`;
